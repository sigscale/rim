...
bulk_cm_5gc(Config) ->
	...
	#xmlElement{content = _SeppEpSbiXContent,
			attributes = SeppEpSbiXAttr} = lists:keyfind('ngc:EP_SBI_X',
			#xmlElement.name, SeppContent),
	#xmlAttribute{value = SeppEpSbiXId} = lists:keyfind(id,
			#xmlAttribute.name, SeppEpSbiXAttr),
	...
	SeppEpN32Name = lists:flatten([DnPrefix, ",SubNetwork=", SubnetId,
			",ManagedElement=", MeId, ",SEPPFunction=", SeppId,
			",EP_N32=", SeppEpN32Id]),
	SeppEpSbiXName = lists:flatten([DnPrefix, ",SubNetwork=", SubnetId,
			",ManagedElement=", MeId, ",SEPPFunction=", SeppId,
			",EP_SBI_X=", SeppEpSbiXId]),
	{ok, #resource{name = SeppName}} = im:get_resource_name(SeppName),
	{ok, #resource{name = SeppEpN32Name}} = im:get_resource_name(SeppEpN32Name),
	{ok, #resource{name = SeppEpSbiXName}} = im:get_resource_name(SeppEpSbiXName),
