...
%%
%% The following environment variables must be configured in the {@link //im. im} application.
%%
%% <dl>
...
%% @hidden
validate_payload1(#{"aud" := Aud, "iss" := Iss, "exp" := Exp} = DecodedPayload)
		when is_list(Iss) ->
	{ok, Audience} = application:get_env(im, oauth_audience),
	true = check_aud(Audience, Aud),
	{ok, IssValue} = application:get_env(im, oauth_issuer),
	true = IssValue == Iss,
	true = os:system_time(seconds) < Exp,
	{valid, DecodedPayload};
validate_payload1(_DecodedPayload) ->
	{error, invalid}.

%% @hidden
check_aud(Aud, AudMap) when is_map(AudMap) ->
	maps:is_key(Aud, AudMap);
...
validate_header(EncodedHeader) ->
	...
	case zj:decode(decode_base64url(EncodedHeader1)) of
		{ok, #{"alg" := "RS256", "typ" := "JWT"} = DecodedHeader} ->
			{valid, DecodedHeader};
		_ ->
			{error, invalid}
	end.
